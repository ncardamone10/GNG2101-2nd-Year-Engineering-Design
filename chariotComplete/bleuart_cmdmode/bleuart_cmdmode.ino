...
void loop(void)
{
  ...
  if (received=="forward" || received=="f") {
    rightmotor.run(FORWARD);
    leftmotor.run(FORWARD);
  } else if (received=="backward" || received=="b") {
    rightmotor.run(BACKWARD);
    leftmotor.run(BACKWARD);
  } else if (received=="left" || received=="l") {
    rightmotor.run(FORWARD);
    leftmotor.run(BACKWARD);
    delay(250);
    rightmotor.run(RELEASE);
    leftmotor.run(RELEASE);
  } else if (received=="right" || received=="r") {
    rightmotor.run(BACKWARD);
    leftmotor.run(FORWARD);
    delay(250);
    rightmotor.run(RELEASE);
    leftmotor.run(RELEASE);
  } else if (received=="stop" || received=="s") {
    rightmotor.run(RELEASE);
    leftmotor.run(RELEASE);
  }
  ble.waitForOK();
